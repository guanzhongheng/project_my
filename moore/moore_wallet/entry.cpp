...
public:
    ...
    bool deserialize_json (bool & upgraded_a, boost::property_tree::ptree & tree_a)
    {
        ...
        if (!tree_a.empty ())
        {
            ...
            language = tree_a.get<std::string> ("language");
...
{
    QMessageBox message (QMessageBox::Critical, QObject::tr("Error starting Moore"), message_a.c_str ());
...
bool update_config (qt_wallet_config & config_a, boost::filesystem::path const & config_path_a, std::fstream & config_file_a)
{
    ...
    auto language (config_a.language);
    auto error (false);
    if (!rai::fetch_object (config_a, config_path_a, config_file_a))
    {
        if (account != config_a.account || wallet != config_a.wallet || language != config_a.language)
        {
            config_a.account = account;
            config_a.wallet = wallet;
            config_a.language = language;
            config_file_a.close ();
...
void initTranslations(QTranslator &qtTranslator, QApplication & application, std::string &lang)
{
	QApplication::removeTranslator(&qtTranslator);
	QString translationFile;
	if (lang == "en"){
		translationFile = ":/locale/moore_en.qm";
	}else{
		translationFile = ":/locale/moore_zh_CN.qm";
	}
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path)
{
	...
	if (!error)
	{
		...
		initTranslations(qtTranslator, application, config.language);
		...
		std::shared_ptr<moore_qt::wallet> gui;
		...
		if (!init.error ())
		{
			...
			application.postEvent (&processor, new moore_qt::eventloop_event ([&]() {
                gui = std::make_shared<moore_qt::wallet> (application, processor, *node, wallet, config.account, config.language);
				splash->close ();
				gui->start ();
	...
	{
		splash->showMessage (QSplashScreen::tr ("Error deserializing config"), Qt::AlignBottom | Qt::AlignHCenter, Qt::darkGray);
		application.processEvents ();
