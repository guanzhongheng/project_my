...
#include <moore/node/xorshift.hpp>
...
opencl (opencl_a)
{
	...
	auto count (rai::moore_network == rai::moore_networks::moore_test_network ? 1 : std::min (max_threads_a, std::max (1u, std::thread::hardware_concurrency ())));
