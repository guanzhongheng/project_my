...
receive_minimum (1), // add by harry
inactive_supply (0),
...
{
	switch (rai::moore_network)
	{
		case rai::moore_networks::moore_test_network:
			preconfigured_representatives.push_back (rai::genesis_account);
			break;
		case rai::moore_networks::moore_beta_network:
			preconfigured_peers.push_back ("peers-beta.un75.com");
			preconfigured_peers.push_back ("peers-beta.utox.me");
            preconfigured_peers.push_back ("peers-beta.mol.one");
			...
			break;
		case rai::moore_networks::moore_live_network:
			preconfigured_peers.push_back ("peers.un75.com");
			preconfigured_peers.push_back ("peers.utox.me");
            preconfigured_peers.push_back ("peers.mol.one");
			preconfigured_representatives.push_back (rai::account ("52CE00155776821E6FF66E05269B58E27A1B07251D98D79F59C0DE8C2528BACD"));
			preconfigured_representatives.push_back (rai::account ("82A0386E5A7B2DC54862F5B294C3F97B0C32EA4D17B3DC964D8677309A28432C"));
			...
			state_block_parse_canary = rai::block_hash ("689942CA3ADACC6A86294C9F4D46C88576DA0537DC2A733E74F78FCBBEFF0F9C");
			state_block_generate_canary = rai::block_hash ("2FF8DD58B092D207127024AD8C27801EF39C4FFBA3C94F572BFDA32135F8529D");
			break;
		default:
			assert (false);
			break;
...
bool rai::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	...
	switch (version)
	{
		...
		case 3:
			...
			tree_a.put ("receive_minimum", rai::mol_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
...
online_reps (*this)
{
	...
	BOOST_LOG (log) << "Node starting, version: " << MOORE_VERSION_MAJOR << "." << MOORE_VERSION_MINOR;
	...
	if (rai::moore_network == rai::moore_networks::moore_live_network)
	{
        extern const char moore_bootstrap_weights[];
		extern const size_t moore_bootstrap_weights_size;
		rai::bufferstream weight_stream ((const uint8_t *)moore_bootstrap_weights, moore_bootstrap_weights_size);
		...
		if (!rai::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (Mmol_ratio, 0, true) << " MOL";
...
void rai::gap_cache::vote (std::shared_ptr<rai::vote> vote_a)
{
	...
	if (existing != blocks.get<1> ().end ())
	{
		...
		if (winner.first > bootstrap_threshold (transaction))
		{
			...
			auto now (std::chrono::steady_clock::now ());
			node.alarm.add (rai::moore_network == rai::moore_networks::moore_test_network ? now + std::chrono::milliseconds (5) : now + std::chrono::seconds (5), [node_l, hash]() {
...
{
	assert (balance_a >= amount_a * rai::Gmol_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= rai::Gmol_ratio;
		auto balance_scaled ((balance_l / rai::Mmol_ratio).convert_to<double> ());
...
bool rai::reserved_address (rai::endpoint const & endpoint_a)
{
	...
	else if (bytes.is_loopback () && rai::moore_network != rai::moore_networks::moore_test_network)
	{
		result = true;
	}
	else if (bytes >= ipv4_loopback_min && bytes <= ipv4_loopback_max && rai::moore_network != rai::moore_networks::moore_test_network)
	{
		result = true;
	}
	return result;
...
bool rai::election::vote (std::shared_ptr<rai::vote> vote_a)
{
	...
	if (rai::moore_network == rai::moore_networks::moore_test_network || weight > supply / 1000) // 0.1% or above
	{
...
{
	if (rai::moore_network != rai::moore_networks::moore_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
