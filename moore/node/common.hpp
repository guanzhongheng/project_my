...
public:
	...
	static std::array<uint8_t, 2> constexpr magic_number = rai::moore_network == rai::moore_networks::moore_test_network ? std::array<uint8_t, 2> ({ 'B', 'Z' }) : rai::moore_network == rai::moore_networks::moore_beta_network ? std::array<uint8_t, 2> ({ 'B', 'Y' }) : std::array<uint8_t, 2> ({ 'B', 'X' });
	uint8_t version_max;
