#include <moore/node/openclwork.hpp>

#include <moore/lib/utility.hpp>
...
__kernel void moore_work (__global ulong * attempt, __global ulong * result_a, __global uchar * item_a)
{
							...
							if (!error_a)
							{
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										kernel = clCreateKernel (program, "moore_work", &kernel_error);
