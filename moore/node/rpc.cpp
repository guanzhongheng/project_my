...
#ifdef MOORE_SECURE_RPC
#include <moore/node/rpc_secure.hpp>
#endif
...
void rai::rpc_handler::mrai_from_raw ()
{
	...
	{
		auto result (amount.number () / rai::Mmol_ratio);
		boost::property_tree::ptree response_l;
...
void rai::rpc_handler::krai_from_raw ()
{
	...
	if (!amount.decode_dec (amount_text))
	{
		auto result (amount.number () / rai::kmol_ratio);
		boost::property_tree::ptree response_l;
		response_l.put ("amount", result.convert_to<std::string> ());
		response (response_l);
	}
...
void rai::rpc_handler::rai_from_raw ()
{
	...
	if (!amount.decode_dec (amount_text))
	{
		auto result (amount.number () / rai::mol_ratio);
		boost::property_tree::ptree response_l;
		response_l.put ("amount", result.convert_to<std::string> ());
		response (response_l);
	}
...
void rai::rpc_handler::version ()
{
	...
	response_l.put ("node_vendor", boost::str (boost::format ("Moore %1%.%2%") % MOORE_VERSION_MAJOR % MOORE_VERSION_MINOR));
	response (response_l);
}
