...
TEST (wallet, send_race)
{
	...
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (rai::test_genesis_key.pub, key2.pub, rai::Gmol_ratio));
		ASSERT_EQ (rai::genesis_amount - rai::Gmol_ratio * i, system.nodes[0]->balance (rai::test_genesis_key.pub));
	}
}
...
TEST (wallet, state_implicit_generate)
{
	...
	system.nodes[0]->ledger.state_block_parse_canary = genesis.hash ();
	{
		...
		rai::state_block block (rai::test_genesis_key.pub, genesis.hash (), rai::test_genesis_key.pub, rai::genesis_amount - rai::Gmol_ratio, rai::test_genesis_key.pub, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
