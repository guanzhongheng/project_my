...
TEST (wallet, construction)
{
    ...
    auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key, language));
	wallet->start ();
...
TEST (wallet, status)
{
	...
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub, language));
	wallet->start ();
	auto wallet_has = [wallet](moore_qt::status_types status_ty) {
	...
	ASSERT_FALSE (wallet_has (moore_qt::status_types::synchronizing));
	auto iterations (0);
	while (!wallet_has (moore_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (moore_qt::status_types::disconnected));
}
...
TEST (wallet, startup_balance)
{
	...
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub, language));
	wallet->start ();
	ASSERT_EQ ("Balance: 0 MOL", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1, language));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account, language));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		rai::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
...
TEST (history, short_text)
{
	...
	moore_qt::history history (ledger, rai::test_genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (wallet, import)
{
	...
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub, language));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	system.wallet (0)->insert_adhoc (key1.prv);
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub, language));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, change_seed)
{
	...
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key, language));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	auto key1 (system.wallet (0)->deterministic_insert ());
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1, language));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, synchronizing)
{
	...
	std::string language = "zh_CN";
	auto wallet (std::make_shared<moore_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1, language));
	wallet->start ();
	{
	...
	ASSERT_EQ (0, wallet->active_status.active.count (moore_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	auto iterations0 (0);
	while (wallet->active_status.active.count (moore_qt::status_types::synchronizing) == 0)
	{
		system0.poll ();
		system1.poll ();
		test_application->processEvents ();
	...
	auto iterations1 (0);
	while (wallet->active_status.active.count (moore_qt::status_types::synchronizing) == 1)
	{
		system0.poll ();
		system1.poll ();
		test_application->processEvents ();
