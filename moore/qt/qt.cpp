...
bool moore_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<moore_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<moore_qt::eventloop_event *> (event_a)->action ();
	return true;
}

moore_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

moore_qt::self_pane::self_pane (moore_qt::wallet & wallet_a, rai::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel (QObject::tr("Account:"))),
account_window (new QWidget),
account_layout (new QHBoxLayout),
account_text (new QLineEdit),
copy_button (new QPushButton (QObject::tr("Copy"))),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % MOORE_VERSION_MAJOR % MOORE_VERSION_MINOR).c_str ());
	...
	QObject::connect (copy_button, &QPushButton::clicked, [this]() {
		...
		copy_button->setText (QObject::tr("Copied!"));
		this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (2), [this]() {
			this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
				copy_button->setText (QObject::tr("Copy"));
			}));
		});
	});
}

void moore_qt::self_pane::refresh_balance ()
{
	auto balance (wallet.node.balance_pending (wallet.account));
	auto final_text (std::string ("Balance(余额): ") + wallet.format_balance (balance.first));
	if (!balance.second.is_zero ())
	{
        final_text += "\nPending(等待): " + wallet.format_balance (balance.second);
	}
	wallet.self.balance_label->setText (QString (final_text.c_str ()));
}

moore_qt::accounts::accounts (moore_qt::wallet & wallet_a) :
window (new QWidget),
wallet_balance_label (new QLabel),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
use_account (new QPushButton (QObject::tr("Use account"))),
create_account (new QPushButton (QObject::tr("Create account"))),
import_wallet (new QPushButton (QObject::tr("Import wallet"))),
backup_seed (new QPushButton (QObject::tr("Copy wallet seed to clipboard"))),
separator (new QFrame),
account_key_line (new QLineEdit),
account_key_button (new QPushButton (QObject::tr("Import adhoc key"))),
back (new QPushButton (QObject::tr("Back"))),
wallet (wallet_a)
{
	separator->setFrameShape (QFrame::HLine);
	separator->setFrameShadow (QFrame::Sunken);
    model->setHorizontalHeaderItem (0, new QStandardItem (QObject::tr("Balance")));
    model->setHorizontalHeaderItem (1, new QStandardItem (QObject::tr("Account")));
	...
	QObject::connect (create_account, &QPushButton::released, [this]() {
		...
		else
		{
            ...
            create_account->setText (QObject::tr("Wallet is locked, unlock it to create account"));
			this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
				this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
					show_button_ok (*create_account);
                    create_account->setText (QObject::tr("Create account"));
	...
	QObject::connect (backup_seed, &QPushButton::released, [this]() {
		...
		if (this->wallet.wallet_m->store.valid_password (transaction))
		{
            ...
            backup_seed->setText (QObject::tr("Seed was copied to clipboard"));
			this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
				this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
					show_button_ok (*backup_seed);
                    backup_seed->setText (QObject::tr("Copy wallet seed to clipboard"));
				}));
			});
		}
		else
		{
            ...
            backup_seed->setText (QObject::tr("Wallet is locked, unlock it to enable the backup"));
			this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
				this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
					show_button_ok (*backup_seed);
                    backup_seed->setText (QObject::tr("Copy wallet seed to clipboard"));
...
void moore_qt::accounts::refresh_wallet_balance ()
{
	rai::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
	...
	auto final_text (std::string ("Balance(余额): ") + wallet.format_balance (balance));
	if (!pending.is_zero ())
	{
        final_text += "\nPending(等待): " + wallet.format_balance (pending);
	}
...
void moore_qt::accounts::refresh ()
{
...
moore_qt::import::import (moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
seed_label (new QLabel (QObject::tr("Seed:"))),
seed (new QLineEdit),
clear_label (new QLabel (QObject::tr("Modifying seed clears existing keys\nType 'clear keys' below to confirm:"))),
clear_line (new QLineEdit),
import_seed (new QPushButton (QObject::tr("Import seed"))),
separator (new QFrame),
filename_label (new QLabel (QObject::tr("Path to file:"))),
filename (new QLineEdit),
password_label (new QLabel (QObject::tr("Password:"))),
password (new QLineEdit),
perform (new QPushButton (QObject::tr("Import"))),
back (new QPushButton (QObject::tr("Back"))),
wallet (wallet_a)
{
    ...
    clear_line->setPlaceholderText (QObject::tr("clear keys"));
	...
	QObject::connect (import_seed, &QPushButton::released, [this]() {
		if (clear_line->text ().toStdString () == "clear keys")
		{
			...
			if (!seed_l.data.decode_hex (seed->text ().toStdString ()))
			{
				bool successful (false);
				{
					...
					else
					{
                        ...
                        import_seed->setText (QObject::tr("Wallet is locked, unlock it to enable the import"));
						this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (10), [this]() {
							this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
								show_line_ok (*seed);
								show_button_ok (*import_seed);
                                import_seed->setText (QObject::tr("Import seed"));
							}));
						});
					}
				}
				if (successful)
				{
                    ...
                    import_seed->setText (QObject::tr("Successful import of seed"));
					this->wallet.refresh ();
					this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
						this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
							show_button_ok (*import_seed);
			...
			else
			{
				...
				{
                    import_seed->setText (QObject::tr("Incorrect seed, length must be 64"));
				}
				else
				{
                    import_seed->setText (QObject::tr("Incorrect seed. Only HEX characters allowed"));
				}
				this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
					this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
						show_button_ok (*import_seed);
		...
		else
		{
			...
			show_button_error (*import_seed);
            import_seed->setText (QObject::tr("Type words 'clear keys'"));
			this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
				this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
					show_button_ok (*import_seed);
                    import_seed->setText (QObject::tr("Import seed"));
...
moore_qt::history::history (rai::ledger & ledger_a, rai::account const & account_a, moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
tx_label (new QLabel (QObject::tr("Account history count:"))),
...
{ /*
    ...
    model->setHorizontalHeaderItem (0, new QStandardItem (QObject::tr("Type")));
    model->setHorizontalHeaderItem (1, new QStandardItem (QObject::tr("Account")));
    model->setHorizontalHeaderItem (2, new QStandardItem (QObject::tr("Amount")));
    model->setHorizontalHeaderItem (3, new QStandardItem (QObject::tr("Hash")));
	view->setModel (model);
	view->setEditTriggers (QAbstractItemView::NoEditTriggers);
	view->verticalHeader ()->hide ();
	view->horizontalHeader ()->setStretchLastSection (true);
...
moore_qt::block_viewer::block_viewer (moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
hash_label (new QLabel (QObject::tr("Hash:"))),
hash (new QLineEdit),
block_label (new QLabel (QObject::tr("Block:"))),
block (new QPlainTextEdit),
successor_label (new QLabel (QObject::tr("Successor:"))),
successor (new QLineEdit),
retrieve (new QPushButton (QObject::tr("Retrieve"))),
rebroadcast (new QPushButton (QObject::tr("Rebroadcast"))),
back (new QPushButton (QObject::tr("Back"))),
wallet (wallet_a)
{
	...
	QObject::connect (retrieve, &QPushButton::released, [this]() {
		...
		if (!hash_l.decode_hex (hash->text ().toStdString ()))
		{
			...
			{
                block->setPlainText (QObject::tr("Block not found"));
			}
		}
		else
		{
            block->setPlainText (QObject::tr("Bad block hash"));
		}
	});
    ...
    rebroadcast->setToolTip (QObject::tr("Rebroadcast block into the network"));
}

void moore_qt::block_viewer::rebroadcast_action (rai::uint256_union const & hash_a)
{
	auto done (true);
...
moore_qt::account_viewer::account_viewer (moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
account_label (new QLabel (QObject::tr("Account:"))),
account_line (new QLineEdit),
refresh (new QPushButton (QObject::tr("Refresh"))),
balance_window (new QWidget),
balance_layout (new QHBoxLayout),
balance_label (new QLabel),
...
moore_qt::status::status (moore_qt::wallet & wallet_a) :
wallet (wallet_a)
{
    wallet.status->setToolTip (QObject::tr("Wallet status, block count (blocks downloaded)"));
	active.insert (moore_qt::status_types::nominal);
	set_text ();
}

void moore_qt::status::erase (moore_qt::status_types status_a)
{
	assert (status_a != moore_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void moore_qt::status::insert (moore_qt::status_types status_a)
{
	assert (status_a != moore_qt::status_types::nominal);
...
std::string moore_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case moore_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case moore_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case moore_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case moore_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case moore_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case moore_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case moore_qt::status_types::nominal:
...
std::string moore_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
moore_qt::wallet::wallet (QApplication & application_a, moore_qt::eventloop_processor & processor_a, rai::node & node_a, std::shared_ptr<rai::wallet> wallet_a, rai::account & account_a, std::string & language_a) :
rendering_ratio (rai::mol_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
language (language_a),
...
separator (new QFrame),
account_history_label (new QLabel (QObject::tr("Account history:"))),
send_blocks (new QPushButton (QObject::tr("Send"))),
settings_button (new QPushButton (QObject::tr("Settings"))),
accounts_button (new QPushButton (QObject::tr("Accounts"))),
show_advanced (new QPushButton (QObject::tr("Advanced"))),
send_blocks_window (new QWidget),
send_blocks_layout (new QVBoxLayout),
send_account_label (new QLabel (QObject::tr("Destination account:"))),
send_account (new QLineEdit),
send_count_label (new QLabel (QObject::tr("Amount:"))),
send_count (new QLineEdit),
send_blocks_send (new QPushButton (QObject::tr("Send"))),
send_blocks_back (new QPushButton (QObject::tr("Back"))),
active_status (*this)
{
	update_connected ();
	empty_password ();
	...
	client_window->resize (320, 640);
	client_window->setStyleSheet ("\
		QLineEdit { padding: 3px; } \
	");
    client_window->setWindowTitle(QObject::tr("Mol Wallet"));
	refresh ();
}

void moore_qt::wallet::start ()
{
	std::weak_ptr<moore_qt::wallet> this_w (shared_from_this ());
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			if (!amount.decode_dec (this_l->send_count->text ().toStdString ()))
			{
				...
				if (actual / this_l->rendering_ratio == amount.number ())
				{
					...
					if (!parse_error)
					{
						...
						if (actual <= balance)
						{
							...
							else
							{
                                ...
                                this_l->send_blocks_send->setText (QObject::tr("Wallet is locked, unlock it to send"));
						...
						else
						{
							show_line_error (*this_l->send_count);
							show_button_error (*this_l->send_blocks_send);
                            this_l->send_blocks_send->setText (QObject::tr("Not enough balance"));
							this_l->node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this_w]() {
								if (auto this_l = this_w.lock ())
								{
									this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
										...
										{
											show_button_ok (*this_l->send_blocks_send);
                                            this_l->send_blocks_send->setText (QObject::tr("Send"));
										}
									}));
								}
							});
					...
					else
					{
						...
						show_button_error (*this_l->send_blocks_send);
                        this_l->send_blocks_send->setText (QObject::tr("Bad destination account"));
						this_l->node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this_w]() {
							if (auto this_l = this_w.lock ())
							{
								this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				...
				else
				{
					show_line_error (*this_l->send_count);
					show_button_error (*this_l->send_blocks_send);
                    this_l->send_blocks_send->setText (QObject::tr("Amount too big"));
					this_l->node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this_w]() {
						if (auto this_l = this_w.lock ())
						{
							show_line_ok (*this_l->send_account);
			...
			else
			{
				show_line_error (*this_l->send_count);
				show_button_error (*this_l->send_blocks_send);
                this_l->send_blocks_send->setText (QObject::tr("Bad amount number"));
				this_l->node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this_w]() {
					if (auto this_l = this_w.lock ())
					{
						this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (moore_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (moore_qt::status_types::active);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (moore_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (moore_qt::status_types::synchronizing);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (moore_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (moore_qt::status_types::working);
					}
				}
			}));
		}
    ...
    settings_button->setToolTip (QObject::tr("Unlock wallet, set password, change representative"));
}

void moore_qt::wallet::refresh ()
{
	{
...
void moore_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (moore_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (moore_qt::status_types::disconnected);
	}
}

void moore_qt::wallet::empty_password ()
{
...
void moore_qt::wallet::change_rendering_ratio (rai::uint128_t const & rendering_ratio_a)
{
...
std::string moore_qt::wallet::format_balance (rai::uint128_t const & balance) const
{
	auto balance_str = rai::amount (balance).format_balance (rendering_ratio, 0, false);
	auto unit = std::string ("MOL");
	if (rendering_ratio == rai::kmol_ratio)
	{
		unit = std::string ("kmol");
	}
	else if (rendering_ratio == rai::mol_ratio)
	{
		unit = std::string ("mol");
	}
...
void moore_qt::wallet::change_language()
{
    if (language == "en") language = "zh_CN";
    else language = "en";
    advanced.language_change->setText (QObject::tr("Please restart required to activate changes"));
    show_button_error(*advanced.language_change);
    advanced.language_change->setDisabled(1);
}

void moore_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void moore_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

moore_qt::settings::settings (moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
lock_toggle (new QPushButton (QObject::tr("Unlock"))),
sep1 (new QFrame),
new_password (new QLineEdit),
retype_password (new QLineEdit),
change (new QPushButton (QObject::tr("Set/Change password"))),
sep2 (new QFrame),
representative (new QLabel (QObject::tr("Account representative:"))),
current_representative (new QLabel),
new_representative (new QLineEdit),
change_rep (new QPushButton (QObject::tr("Change representative"))),
back (new QPushButton (QObject::tr("Back"))),
wallet (wallet_a)
{
    password->setPlaceholderText (QObject::tr("Password"));
    ...
    new_password->setPlaceholderText (QObject::tr("New password"));
	layout->addWidget (new_password);
	retype_password->setEchoMode (QLineEdit::EchoMode::Password);
    retype_password->setPlaceholderText (QObject::tr("Retype password"));
	...
	QObject::connect (change, &QPushButton::released, [this]() {
		...
		if (this->wallet.wallet_m->store.valid_password (transaction))
		{
			...
			{
				new_password->clear ();
                new_password->setPlaceholderText (QObject::tr("Empty Password - try again: New password"));
				retype_password->clear ();
                retype_password->setPlaceholderText (QObject::tr("Empty Password - try again: Retype password"));
			}
			else
			{
				if (new_password->text () == retype_password->text ())
				{
                    ...
                    change->setText (QObject::tr("Password was changed"));
					update_locked (false, false);
					this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
						this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
							show_button_ok (*change);
                            change->setText (QObject::tr("Set/Change password"));
						}));
					});
				}
				else
				{
					retype_password->clear ();
                    retype_password->setPlaceholderText (QObject::tr("Password mismatch"));
				}
			}
		}
		else
		{
            ...
            change->setText (QObject::tr("Wallet is locked, unlock it"));
			this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
				this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
					show_button_ok (*change);
                    change->setText (QObject::tr("Set/Change password"));
	...
	QObject::connect (change_rep, &QPushButton::released, [this]() {
		...
		if (!representative_l.decode_account (new_representative->text ().toStdString ()))
		{
			...
			if (this->wallet.wallet_m->store.valid_password (transaction))
			{
                ...
                change_rep->setText (QObject::tr("Representative was changed"));
				...
				this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
					this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
                        ...
                        change_rep->setText (QObject::tr("Change representative"));
					}));
				});
			}
			else
			{
                ...
                change_rep->setText (QObject::tr("Wallet is locked, unlock it"));
				this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
					this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
						show_button_ok (*change_rep);
                        change_rep->setText (QObject::tr("Change representative"));
		...
		else
		{
			...
			show_button_error (*change_rep);
            change_rep->setText (QObject::tr("Invalid account"));
			this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
				this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
	...
	QObject::connect (lock_toggle, &QPushButton::released, [this]() {
		...
		if (this->wallet.wallet_m->store.valid_password (transaction))
		{
            ...
            lock_toggle->setText (QObject::tr("Unlock"));
		...
		else
		{
			...
			{
				password->clear ();
                lock_toggle->setText (QObject::tr("Lock"));
			...
			else
			{
                ...
                lock_toggle->setText (QObject::tr("Invalid password"));
				this->wallet.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::seconds (5), [this]() {
					this->wallet.application.postEvent (&this->wallet.processor, new eventloop_event ([this]() {
						show_line_ok (*password);
...
void moore_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void moore_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (moore_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (moore_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (moore_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (moore_qt::status_types::vulnerable);
	}
}

moore_qt::advanced_actions::advanced_actions (moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
show_ledger (new QPushButton (QObject::tr("Ledger"))),
show_peers (new QPushButton (QObject::tr("Peers"))),
language_change (new QPushButton (QObject::tr("切换/中文"))),
bootstrap (new QPushButton (QObject::tr("Initiate bootstrap"))),
wallet_refresh (new QPushButton (QObject::tr("Refresh Wallet"))),
create_block (new QPushButton (QObject::tr("Create Block"))),
enter_block (new QPushButton (QObject::tr("Enter Block"))),
block_viewer (new QPushButton (QObject::tr("Block Viewer"))),
account_viewer (new QPushButton (QObject::tr("Account Viewer"))),
scale_window (new QWidget),
scale_layout (new QHBoxLayout),
scale_label (new QLabel (QObject::tr("Scale:"))),
ratio_group (new QButtonGroup),
mrai (new QRadioButton ("Mmol")),
krai (new QRadioButton ("kmol")),
rai (new QRadioButton ("mol")),
back (new QPushButton (QObject::tr("Back"))),
ledger_window (new QWidget),
ledger_layout (new QVBoxLayout),
ledger_model (new QStandardItemModel),
ledger_view (new QTableView),
ledger_refresh (new QPushButton (QObject::tr("Refresh"))),
ledger_back (new QPushButton (QObject::tr("Back"))),
peers_window (new QWidget),
peers_layout (new QVBoxLayout),
peers_model (new QStandardItemModel),
peers_view (new QTableView),
bootstrap_label (new QLabel (QObject::tr("IPV6:port \"::ffff:192.168.0.1:17075\""))),
bootstrap_line (new QLineEdit),
peers_bootstrap (new QPushButton (QObject::tr("Initiate Bootstrap"))),
peers_refresh (new QPushButton (QObject::tr("Refresh"))),
peers_back (new QPushButton (QObject::tr("Back"))),
wallet (wallet_a)
{
    ...
    ledger_model->setHorizontalHeaderItem (0, new QStandardItem (QObject::tr("Account")));
    ledger_model->setHorizontalHeaderItem (1, new QStandardItem (QObject::tr("Balance")));
    ledger_model->setHorizontalHeaderItem (2, new QStandardItem (QObject::tr("Block")));
    ...
    peers_model->setHorizontalHeaderItem (0, new QStandardItem (QObject::tr("IPv6 address:port")));
    peers_model->setHorizontalHeaderItem (1, new QStandardItem (QObject::tr("Net version")));
    ...
    layout->addWidget (language_change);
	...
	QObject::connect (mrai, &QRadioButton::toggled, [this]() {
		if (mrai->isChecked ())
		{
			this->wallet.change_rendering_ratio (rai::Mmol_ratio);
		}
	});
	QObject::connect (krai, &QRadioButton::toggled, [this]() {
		if (krai->isChecked ())
		{
			this->wallet.change_rendering_ratio (rai::kmol_ratio);
		}
	});
	QObject::connect (rai, &QRadioButton::toggled, [this]() {
		if (rai->isChecked ())
		{
			this->wallet.change_rendering_ratio (rai::mol_ratio);
		}
	});
	mrai->click ();
    ...
    QObject::connect (language_change, &QPushButton::released, [this]() {              
        this->wallet.change_language ();
	});
    ...
    bootstrap->setToolTip (QObject::tr("Multi-connection bootstrap to random peers"));
    create_block->setToolTip (QObject::tr("Create block in JSON format"));
    enter_block->setToolTip (QObject::tr("Enter block in JSON format"));
}

void moore_qt::advanced_actions::refresh_peers ()
{
...
void moore_qt::advanced_actions::refresh_ledger ()
{
...
moore_qt::block_entry::block_entry (moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
process (new QPushButton (QObject::tr("Process"))),
back (new QPushButton (QObject::tr("Back"))),
wallet (wallet_a)
{
	layout->addWidget (block);
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			else
			{
                ...
                this->status->setText (QObject::tr("Unable to parse block"));
			}
		}
...
moore_qt::block_creation::block_creation (moore_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
group (new QButtonGroup),
button_layout (new QHBoxLayout),
send (new QRadioButton (QObject::tr("Send"))),
receive (new QRadioButton (QObject::tr("Receive"))),
change (new QRadioButton (QObject::tr("Change"))),
open (new QRadioButton (QObject::tr("Open"))),
account_label (new QLabel (QObject::tr("Account:"))),
account (new QLineEdit),
source_label (new QLabel (QObject::tr("Source:"))),
source (new QLineEdit),
amount_label (new QLabel (QObject::tr("Amount:"))),
amount (new QLineEdit),
destination_label (new QLabel (QObject::tr("Destination:"))),
destination (new QLineEdit),
representative_label (new QLabel (QObject::tr("Representative:"))),
representative (new QLineEdit),
block (new QPlainTextEdit),
status (new QLabel),
create (new QPushButton (QObject::tr("Create"))),
back (new QPushButton (QObject::tr("Back"))),
wallet (wallet_a)
{
...
void moore_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void moore_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void moore_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void moore_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void moore_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void moore_qt::block_creation::create_send ()
{
	rai::account account_l;
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						show_label_ok (*status);
                        status->setText (QObject::tr("Created block"));
					}
					else
					{
						show_label_error (*status);
                        status->setText (QObject::tr("Insufficient balance"));
					}
				}
				else
				{
					show_label_error (*status);
                    status->setText (QObject::tr("Account is not in wallet"));
				}
			}
			else
			{
				show_label_error (*status);
                status->setText (QObject::tr("Unable to decode destination"));
			}
		}
		else
		{
			show_label_error (*status);
            status->setText (QObject::tr("Unable to decode amount"));
		}
	}
	else
	{
		show_label_error (*status);
        status->setText (QObject::tr("Unable to decode account"));
	}
}

void moore_qt::block_creation::create_receive ()
{
		...
		if (block_l != nullptr)
		{
			...
			if (!destination.is_zero ())
			{
				...
				if (!wallet.node.store.pending_get (transaction, pending_key, pending))
				{
					...
					{
						show_label_error (*status);
                        status->setText (QObject::tr("Account not yet open"));
					}
				}
				else
				{
					show_label_error (*status);
                    status->setText (QObject::tr("Source block is not pending to receive"));
				}
			}
			else
			{
				show_label_error (*status);
                status->setText (QObject::tr("Source is not a send block"));
			}
		}
		else
		{
			show_label_error (*status);
            status->setText (QObject::tr("Source block not found"));
		}
	}
	else
	{
		show_label_error (*status);
        status->setText (QObject::tr("Unable to decode source"));
	}
}

void moore_qt::block_creation::create_change ()
{
	rai::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		{
			show_label_error (*status);
            status->setText (QObject::tr("Unable to decode representative"));
		}
	}
	else
	{
...
void moore_qt::block_creation::create_open ()
{
	rai::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
			...
			if (block_l != nullptr)
			{
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (transaction, pending_key, pending))
					{
						...
						{
							show_label_error (*status);
                            status->setText (QObject::tr("Account already open"));
						}
					}
					else
					{
