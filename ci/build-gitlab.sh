...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    DISTRO_CFG="-DMOORE_DISTRO_NAME=${distro}"
    ...
    CPACK_TYPE="TBZ2"
    DISTRO_CFG="-DMOORE_DISTRO_NAME='freebsd'"
else
    CPACK_TYPE="TBZ2"
fi

if [[ ${SIMD} -eq 1 ]]; then
    SIMD_CFG="-DMOORE_SIMD_OPTIMIZATIONS=ON"
    CRYPTOPP_CFG=""
...
if [[ ${ASAN_INT} -eq 1 ]]; then
    SANITIZERS="-DMOORE_ASAN_INT=ON"
elif [[ ${ASAN} -eq 1 ]]; then
    SANITIZERS="-DMOORE_ASAN=ON"
elif [[ ${TSAN} -eq 1 ]]; then
    SANITIZERS="-DMOORE_TSAN=ON"
else
    SANITIZERS=""
fi
